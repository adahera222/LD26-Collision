...
public partial class TouchKit
{
	...
	private void debugDrawRect( TKRect rect, Color color )
	{
		...
		/*
		bl = Camera.main.transform.InverseTransformPoint( bl );
		bl = Camera.main.ScreenToWorldPoint( bl );
		br = Camera.main.transform.InverseTransformPoint( br );
		br = Camera.main.ScreenToWorldPoint( br );
		tl = Camera.main.transform.InverseTransformPoint( tl );
		tl = Camera.main.ScreenToWorldPoint( tl );
		tr = Camera.main.transform.InverseTransformPoint( tr );
		tr = Camera.main.ScreenToWorldPoint( tr );
		*/
		
		/*bl = M42.ScreenToWorldPos(bl);
		br = M42.ScreenToWorldPos(br);
		tl = M42.ScreenToWorldPos(tl);
		tr = M42.ScreenToWorldPos(tr);*/
